...
from typing import (Any, Dict, Iterable, List, NamedTuple, Tuple, TypedDict,
                    Union)

import yaml
from warnings import warn
from urllib.parse import urlparse
from dilib.core.prefect_flow_config import (SplitgraphKubernetesFlowConfig,
                                            SplitgraphKubernetesFlowReg)
...
class CurrentContext(object):
    '''Borrows code from kedro project.

    The goal is to integrate the data catalog and configuration libraries in a way that is compatible with prefect.
    '''
    def __init__(
        self,
        project_path: Union[Path, str],
        prefect_env: str = os.environ.get('PREFECT_ENV', DEFAULT_PREFECT_ENV),
        extra_params: Dict[str, Any] = None,
        default_remote_name: str = os.environ.get('DEFAULT_REMOTE_NAME', DEFAULT_REMOTE),
        env_namespace_prefix: str = os.environ.get('ENV_NAMESPACE_PREFIX', '')
    ):
    ...
    def project_path(self) -> Path:
        """Read-only property containing root project directory.
        Returns:
            Project directory.
        """
    ...
    def build_standard_config(
        ...
        job_template = yaml.safe_load(pkgutil.get_data("dilib.core", "job_template.yaml"))
    ...
    def _get_config_loader(self) -> ConfigLoader:
        conf_paths = [
            str(self.project_path / 'conf' / 'base'),
            str(self.project_path / 'conf' / self.dilib_context.prefect_env),
        ]
        print(conf_paths)
